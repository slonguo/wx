...
// Fix(slonguo): change include path
// see mysql.BUILD
#include <mysql.h>
#include <errmsg.h>
...
// Fix(slonguo): uncomment the following #define would 
// cause multiple errors when compiling on MacOS 10.15.3 (19D76)
// with clang info:
...
namespace daotk {
	...
	namespace mysql {
		...
		// Fix(slonguo): mysql 8.x discarded `my_bool` type
		...
		inline std::string format_string(const char* fmt_str, ...) {
			...
			va_end(vargs);
			// Fix(slonguo): fix compiler warning compaint
			// return std::move(res);
			return res;
		}
...
//				if (!get_value(i, v)) throw mysqlpp_exception(mysqlpp_exception::empty_result);
		...
		// added(slonguo):
		// see: https://zh.cppreference.com/w/cpp/memory/enable_shared_from_this
		// we'd also need a connection pool(see utils/conn_pool.hpp)
		public:
		...
		public:
			...
			std::vector<result> mquery(const std::string& query_str) {
				...
				// Fix(slonguo): fix clang warning complaint
				// return std::move(res);
				return res;
			}
		...
		public:
			...
			// see: https://zh.cppreference.com/w/cpp/memory/enable_shared_from_this
			public:
			std::shared_ptr<prepared_stmt> getptr() {
					return shared_from_this();
				}
